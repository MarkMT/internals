...
provide support for database stores: DataMapper, Sequel, ActiveRecord. Because DataMapper
can use non-relational databases and basically any data source you can imagine, it should
be possible to actually store sessions in CouchDB, StrokeDB, SimpleDB or similar.
Other than that, there's nothing special in Merb's session functionality implementation,
it's quite clean and flexible thanks to Fabien Franzen aka loob2.


h2. Multiple stores in one application.
...
Sessions support is implemented with 3 modules:

* SessionMixin that is mixed into Merb::Controller.
* Session, that is mixed into session container classes and lets you define your own methods on session object.
* RequestMixin that is mixed into Merb::Request.
...
Merb::Config.session_stores returns array of names of all session store types
used in the application. Each name is a Symbol:

<pre>
irb(main):001:0> Merb::Config.session_stores
=> [:activerecord]
</pre>

You can refer to chapter 2 for configuration options reference.

Piece of code that reveals implementation of what has been described so far is below.
<pre>
<code>
...
Merb::SessionContainer supposed to handle session setup and finalization. Lets discuss a bit what
each of terms mean. Session setup is fetching session id from cookie and loading up session data
from whatever you use for data storage: cookies, database, memcached, VM memory or
something else, then assigning de-serialized session object to request instance.

Session finalization means simply persisting session data. Session container and it's
subclasses also handle session id operations: from uuid generation to changing to clearing.

Session is tied to request object, though controllers get delegating method session from SessionMixin.
This is done to make sure you can use sessions in deferred routes.

Merb::SessionContainer is a base class and supposed to be sub-classed.
<pre>
<code>
module Merb
...
Merb::SessionStoreContainer provides one common API for reading, writing and
clearing session data using session id. Note that
session data hash is memoized on session finalization to prevent container from being
saved multiple times with the same content. This is called session
fingerprinting in Merb. Session fingerprint is rarely needed but still accessible via
request.session._fingerprint if you fall into some edge case.

"Needs new cookie" flag tells if new cookie with session id should be sent to the client
with the next response. If you assign session id explicitly, this is how Merb figures out
that Set-Cookie with sessions id key and value header should be sent to the client.

"Destroy" flag means that session id cookie needs to be set with expiration time = 0 so
client removes it.
<pre>
<code>
module Merb
...
Now when we have idea about session store container API, lets
see what it takes to implement custom session store. It's better
to start with the simplest case: in memory session store.

Memory session store acts pretty much as a Hash with some
additional details: time to live (period of time after which
values in session are wiped out) and mutex-locked access needed
for thread safety. Session access is also thread safe and
synchronized.

Memory session store has a reaper thread that invalidates expired
sessions.

Also, using memory session as example it's easy to see how session object
interacts with aggregated session store. It's actually quite simple and
is true for all types of session (at least provided by merb-core).
...
Keep in mind this session type requires explicit setting of session store because Merb has no
idea what Memcached client you are going to use (memcached gem is faster but is platform dependent
since it's a C extension) and what are hosts/ports of Memcached instances are.

It's usually done in after_app_loads hook but not requires: just make sure it is available
first time session is accessed in your application.
...
Merb forces a 4K size limit on data stores in the session since
this is what most popular browsers accept.

Obviously, session data is marshalled and unmarshalled in case of
cookie session. It's also base64 encrypted for integrity (and not
for security through obscurity). Cookie session store uses
OpenSSL HMAC and SHA1 algorithms for digest and is most likely
to perform worse than memory, memcached or database sessions.
...
DataMapper store uses DataMapper session model that has 3 properties:

* session_id, 32 characters long string
* data, a serialized hash with default value of empty hash.
* creation timestamp
...
Sequel store works just like DM store but uses (you guessed it)
Sequel model.
...
ActiveRecord store uses ActiveRecord model and has no significant
differences with DataMapper and Sequel stores.
...
Now lets see how cookies put together in merb-core. Cookie inherits from Mash
(Hash that can be accessed using both strings and symbols with no difference),
also known as Hash with indifferent access.

Generation of Set-Cookie header is called extraction in Merb and happens once
per request when headers are sent to the client.

To provide access to cookie jar in the controller, CookieMixin is included into
Merb::Controller. RequestMixin provides methods of request that get and set
cookies.
...
Bootloader is where all the mixins get mixed into components they
extend: mostly controller and request, but also session class.

Keep in mind that Merb used to lazy load session class based on
what you specified in init file but no longer: because it supports
multiple stores per application, it's safer to preload them.

As you can see from the code below, default name of session id cookie is
_session_id.

...
Deferred actions is a way to specify which actions in the application take a long time to execute and should be
deferred. The idea is that event-driven web servers (Thin, Ebb) that are usually faster than
Mongrel will spawn a thread to run deferred actions so the event loop is not stopped
and the server can continue serving requests. Usually very useful for file upload actions.
